            ...
            if len(nodules_annotation) > 0:
                ...
                # This current for loop iterates over total number of nodules in a single patient
                    ...
                    for nodule_slice in range(mask.shape[2]):
                        ...
                        if os.path.isfile(patient_mask_dir + '/' +  mask_name + '.npy'):
                        ...
                        # 處理前後影像問題
                        for i in range(nodule_slice+cbbox[2].start-3, nodule_slice+cbbox[2].start+4):
                            nodule_name = "{}_img_slice{}".format(pid[-4:],prefix[i])
                            if os.path.isfile(patient_image_dir + '/' +  nodule_name + '.npy'):
                                continue
                            else :
                                np.save(patient_image_dir + '/' +  nodule_name,vol[:,:,i])
            ...
            for patient in tqdm(self.IDRI_list):
                ...
                if len(nodules_annotation) > 0:
                    ...
                    if start_2 + per_nodule_slice/2 <= vol.shape[2]:
                        ...
                        stop_2 = vol.shape[2]
                    # 把前半部start_1向前推3個, 後半部 stop_2 前後3個
                    if (start_1-3)>=0:
                        start_1 -= 3
                    else : 
                        start_1 = 0
                    if (stop_2+3) <= vol.shape[2]:
                        stop_2 += 3
                    else :
                        stop_2 = vol.shape[2]
                    stop_1 += 3
                    ...
                    per_nodule_slice
                    slice_count = 0
                    if vol.shape[2] >= per_nodule_slice+6:
                        clean_slice = slice(int((vol.shape[2]-per_nodule_slice)/2)-3, int((vol.shape[2]+per_nodule_slice)/2)+3, 1)
                        slice_count = int((vol.shape[2]-per_nodule_slice)/2)-3
                    else:
